...
class DashboardService:
    ...
    def get_bet_statistics(self, days: int = 30) -> Dict[str, Any]:
        db = SessionLocal()
        try:
            since = datetime.utcnow() - timedelta(days=days)
            ...
            total_bets = len(bets)
            unique_users = len(set(bet.user_name for bet in bets))
            ...
            daily_bets = {}
            for bet in bets:
                date = bet.created_at.date()
                daily_bets[date] = daily_bets.get(date, 0) + 1
            ...
            return {
                ...
                "daily_bets": {date.strftime("%Y-%m-%d"): count for date, count in daily_bets.items()}
            }
        finally:
            db.close()

    def get_top_bet_users(self, days: int = 30, limit: int = 10) -> List[Dict[str, Any]]:
        db = SessionLocal()
        try:
            since = datetime.utcnow() - timedelta(days=days)
            ...
            user_stats = (
                db.query(
                    ...
                    func.sum(case((BetHistory.result_type == 'partial', 1), else_=0)).label('partial_matches'),
                    func.sum(case((BetHistory.result_type == 'miss', 1), else_=0)).label('misses'),
                    func.sum(case((BetHistory.rarity_level == RarityLevel.MYTHICAL, 1), else_=0)).label('mythical_count'),
                    func.sum(case((BetHistory.rarity_level == RarityLevel.LEGENDARY, 1), else_=0)).label('legendary_count'),
                    func.sum(case((BetHistory.rarity_level == RarityLevel.EPIC, 1), else_=0)).label('epic_count'),
                    func.sum(case((BetHistory.rarity_level == RarityLevel.RARE, 1), else_=0)).label('rare_count'),
                    func.sum(case((BetHistory.rarity_level == RarityLevel.UNCOMMON, 1), else_=0)).label('uncommon_count'),
                    func.sum(case((BetHistory.rarity_level == RarityLevel.COMMON, 1), else_=0)).label('common_count')
                )
                .filter(BetHistory.channel_name == self.channel_name)
                .filter(BetHistory.created_at >= since)
                .group_by(BetHistory.user_name)
                .order_by(func.count(BetHistory.id).desc())
                .limit(limit)
                .all()
            )
            ...
            return [
                {
                    ...
                    "partial_matches": stat.partial_matches,
                    "misses": stat.misses,
                    "mythical_count": stat.mythical_count,
                    "legendary_count": stat.legendary_count,
                    "epic_count": stat.epic_count,
                    "rare_count": stat.rare_count,
                    "uncommon_count": stat.uncommon_count,
                    "common_count": stat.common_count,
    ...
    def get_rarity_events(self, days: int = 30, rarity_level: RarityLevel = None) -> List[Dict[str, Any]]:
        db = SessionLocal()
        try:
            since = datetime.utcnow() - timedelta(days=days)
            ...
            if rarity_level:
                query = query.filter(BetHistory.rarity_level == rarity_level)

            events = query.order_by(BetHistory.created_at.desc()).all()
