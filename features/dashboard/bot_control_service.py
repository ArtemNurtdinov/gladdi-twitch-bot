...
class BotControlService:
    ...
    def mark_joke_generated(self) -> Dict[str, Any]:
        try:
            success = self.settings_manager.mark_joke_generated()

            if success:
                next_joke_info = self.settings_manager.get_next_joke_info()
                self.logger.info("Анекдот отмечен как сгенерированный")

                return {
                    "success": True,
                    "message": "Анекдот отмечен как сгенерированный",
                    "next_joke": next_joke_info
                }
            else:
                self.logger.warning("Не удалось отметить генерацию анекдота")
                return {
                    "success": False,
                    "message": "Ошибка сохранения информации о генерации",
                    "next_joke": {
                        "enabled": False,
                        "next_joke_time": None,
                        "minutes_until_next": None,
            ...
            self.logger.error(f"Ошибка отметки генерации анекдота: {e}")
            return {
                "success": False,
                "message": f"Ошибка отметки генерации: {str(e)}",
                "next_joke": {
                    "enabled": False,
                    "next_joke_time": None,
                    "minutes_until_next": None,
