...
class ChatRepositoryImpl(ChatRepository):
    ...
    def save(self, message: ChatMessage) -> None:
        self._db.add(
            ChatMessageORM(
                channel_name=message.channel_name, user_name=message.user_name, content=message.content,
    ...
    def top_chat_users(self, limit: int, date_from: datetime | None, date_to: datetime | None) -> Sequence[
        tuple[str, str, int]]:
        ...
        stmt = stmt.group_by(ChatMessageORM.channel_name, ChatMessageORM.user_name).order_by(count_expr.desc()).limit(
            limit)
