...
from app.stream.di.composition import get_stream_query_use_case
...
async def get_streams(
    ...
    try:
        with db_ro_session() as session:
            stream_use_case = get_stream_query_use_case(session)
            items, total = stream_use_case.get_streams(skip, limit, date_from, date_to)
...
async def get_stream_detail(stream_id: int) -> StreamDetailResponse:
    with db_ro_session() as session:
        stream_use_case = get_stream_query_use_case(session)
        dto = stream_use_case.get_stream_detail(stream_id)
