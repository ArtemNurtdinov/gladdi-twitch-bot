...
class ModerationService(ChatModerationPort):
    def __init__(self, platform_repository: PlatformRepository, user_cache: ViewerCachePort, logger: Logger):
    ...
    async def timeout_user(self, channel_name: str, moderator_name: str, username: str, duration_seconds: int, reason: str) -> bool:
        try:
            user_id = await self._user_cache.get_viewer_id(username)
            broadcaster_id = await self._user_cache.get_viewer_id(channel_name)
            moderator_id = await self._user_cache.get_viewer_id(moderator_name)
