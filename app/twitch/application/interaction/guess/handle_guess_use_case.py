    ...
    async def handle_number(
        ...
        if guess == game.target_number:
            ...
            with db_session_provider() as db:
                ...
                self._chat_use_case_factory(db).save_chat_message(dto.channel_name, dto.bot_nick, message, dto.occurred_at)
            return message
        else:
    ...
    async def handle_letter(
        ...
        if not dto.letter_input:
            ...
            if status:
                message = status
                with db_session_provider() as db:
                    self._chat_use_case_factory(db).save_chat_message(dto.channel_name, dto.bot_nick, status, dto.occurred_at)
            else:
        ...
        if letter_revealed:
            ...
            if all_letters_revealed:
                with db_session_provider() as db:
                    winner_balance = self._economy_service_factory(db).add_balance(
                        dto.channel_name,
                        dto.user_name,
                        game.prize_amount,
                        TransactionType.MINIGAME_WIN,
