...
class StatsCommandHandler:
    ...
    async def handle(self, channel_name: str, display_name: str, ctx):
        ...
        with SessionLocal.begin() as db:
            ...
            bets = self._betting_service(db).get_user_bets(channel_name, user_name)
        ...
        with db_ro_session() as db:
            battles = self._battle_use_case(db).get_user_battles(channel_name, display_name)

        if not battles:
            battle_stats = UserBattleStats(total_battles=0, wins=0, losses=0, win_rate=0.0)
        else:
            total_battles = len(battles)
            wins = sum(1 for battle in battles if battle.winner == display_name)
        ...
        result = f"📊 Статистика @{display_name}:  💰 Баланс: {balance.balance} монет."
